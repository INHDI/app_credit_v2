...
import { getNavigationForRole, NavigationSection } from "@/constants/credit-contract";
import { useNavigation } from "@/hooks/useNavigation";
import { useEffect, useState } from "react";
import WebSocketStatusIndicator from "@/components/websocket/WebSocketStatus";
import { useAuth, getRoleDisplayName } from "@/providers/AuthContext";
...
export default function Navigation({ currentPage, className = "" }: NavigationProps) {
  ...
  useEffect(() => {
    if (user) {
      setNavigationSections(getNavigationForRole(user.role));
    } else {
  ...
  return (
    <nav className={`h-full overflow-y-auto py-2 flex flex-col ${className}`}>
      {/* User Info Section */}
              ...
              {section.items.map((item) => {
                ...
                return (
                  <div key={item.id} className="relative">
                    <Button
                      variant="ghost"
                      className={`w-full grid grid-cols-[auto_1fr_auto] items-center h-12 text-left transition-all duration-200 rounded-xl relative overflow-hidden group ${isActive
                    ...
                    >
                      ...
                      <div className={`relative w-9 h-9 rounded-lg flex items-center justify-center transition-all duration-200 ${isActive
                        ? 'bg-white/20 shadow-inner'
                        : 'bg-slate-100 group-hover:bg-white group-hover:shadow-sm'
                        }`}>
                        <IconComponent className={`h-5 w-5 transition-colors duration-200 ${isActive ? 'text-white' : 'text-slate-600 group-hover:text-slate-700'
                          }`} />
                      </div>

                      <div className="flex-1 relative pl-3">
                        <span className={`font-medium text-sm transition-colors duration-200 truncate ${isActive ? 'text-white' : 'text-slate-700 group-hover:text-slate-800'
                          }`}>
                          {item.title}
                        </span>
                      </div>
                      ...
                      <div className="ml-auto flex items-center justify-end w-20 shrink-0 gap-2">
                        ...
                        {section.title === "Quản lý" && (
                          <ChevronRight className={`h-4 w-4 transition-colors duration-200 ${isActive
                            ? 'text-white/80'
      ...
      {/* Logout Button */}
      <div className="px-3 mt-4 pt-4 border-t border-slate-200">
        <Button
          variant="ghost"
          className="w-full flex items-center gap-3 h-12 text-red-600 hover:bg-red-50 hover:text-red-700 rounded-xl transition-all"
...
function getBottomItemsForRole(role: string | null): BottomItem[] {
  if (!role) return [];
...
export function MobileBottomNav() {
  ...
  const items = getBottomItemsForRole(user?.role ?? null);
          ...
          <div className="flex items-end justify-between">
            {items.map((item) => {
              ...
              return (
                ...
                >
                  <div className={`relative w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-200 ${active ? 'bg-teal-100 shadow-inner scale-110' : 'bg-slate-100'}`}>
                  ...
                  <span className={`text-[11px] leading-none h-[14px] ${active ? 'text-teal-600 font-semibold' : 'text-transparent'}`}>{active ? item.title : '.'}</span>
                </button>
              );
            })}
          </div>
