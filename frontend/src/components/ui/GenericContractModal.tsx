...
export default function GenericContractModal({
    ...
    const handleInputChange = (field: FieldConfig, value: any) => {
        setFormData((prev) => {
            ...
            } else if ((field.type === 'number' || field.type === 'currency') && typeof value === 'string') {
                processedValue = parseFloat(value) || 0;
            }

            // Apply custom parsing if provided
            if (field.parseValue && typeof value === 'string') {
                processedValue = field.parseValue(value);
            }

            newFormData[field.key] = processedValue;

            // Handle side effects
            if (field.onSelectSideEffect) {
                const sideEffects = field.onSelectSideEffect(processedValue, newFormData);
                newFormData = { ...newFormData, ...sideEffects };
            }
    ...
    const renderField = (field: FieldConfig) => {
        ...
        switch (field.type) {
                        ...
                        >
                            {field.options?.map((option) => (
                                <label
                                    ...
                                    className={`
                    flex-1 flex items-center justify-center cursor-pointer rounded-xl px-4 py-2.5 text-sm font-medium transition-all duration-200
                    ${formData[field.key] === option.value
                                            ? 'bg-white text-emerald-700 shadow-sm ring-1 ring-black/5'
                                            : 'text-slate-600 hover:bg-slate-200/50 hover:text-slate-900'}
                  `}
                                >
                                    <RadioGroupItem value={option.value} id={`${field.key}-${option.value}`} className="sr-only" />
            ...
            case 'user-search':
                return (
                    <div className="space-y-2 animate-in fade-in slide-in-from-top-1 duration-200 mt-2">
                        <UserSearchInput
                            onSelect={(user) => handleInputChange(field, user)}
                            selectedUser={formData[field.key]}
                            placeholder={field.placeholder}
