...
export default function SettleConfirmModal({
  ...
  const unpaid = useMemo(
    () => items.filter((i) => i.so_tien_tra < i.so_tien),
    [items]
  );
  ...
  const principalAmount =
    contractType === "tin_chap"
      ? (contract?.SoTienVay || 0) - (contract?.SoTienTraGoc || 0)
      : contract?.SoTienVay || 0;

  // Lãi còn lại: ưu tiên LaiConLai từ contract, fallback unpaidTotal
  const interestAmount = contract?.LaiConLai || unpaidTotal;

  const principalValue = principalAmount;

  // Parse số lãi user nhập
  const interestValue = interestInput
    ? parseInt(interestInput.replace(/[^0-9]/g, ""), 10)
    : 0;

  const totalSettlementAmount = principalValue + interestValue;

  // 3) Init interestInput mỗi khi mở modal + interestAmount thay đổi
  useEffect(() => {
    if (!isOpen) return;

    if (interestAmount && interestAmount > 0) {
      setInterestInput(String(interestAmount));
    } else {
      setInterestInput("");
    }
  }, [isOpen, interestAmount]);
          ...
          <div className="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-xl p-4 border border-emerald-200">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  value={interestInput ? formatCurrency(interestValue) : ""}
                  onChange={(e) => {
                    const raw = e.target.value.replace(/[^0-9]/g, "");
                    setInterestInput(raw);
                  }}
                  placeholder="Nhập số tiền lãi..."
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                />
                <p className="text-xs text-slate-500 mt-1">
                  Lãi hiện tại hệ thống tính: {formatCurrency(interestAmount)} | Để
                  trống = không trả lãi
                </p>
              </div>
            </div>
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-200">
            ...
            <div className="space-y-2">
              ...
              {interestValue === 0 && contractType === "tra_gop" && (
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-2 mt-2">
                  <p className="text-xs text-amber-700">ℹ️ Không trả lãi</p>
                </div>
              )}
            </div>
          </div>
